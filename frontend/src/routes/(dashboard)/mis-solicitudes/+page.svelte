<script lang="ts">
	...
	const query = createQuery(() => ({
		queryKey: ['travel-requests', 'my', { page, pageSize }],
		queryFn: () =>
			travelRequestService.getMyTravelRequests({
				page,
				size: pageSize,
...
{#snippet rowActions(row: TravelRequestSummary)}
	<div class="flex items-center justify-end gap-0.5">
		<Button
			href={`${ROUTES.employee.myRequests}/${row.id}`}
			variant="ghost"
			size="icon-sm"
...
<div class="space-y-4">
	<PageHeader
		...
		description="Consulta las solicitudes de viaje que has creado."
	/>
