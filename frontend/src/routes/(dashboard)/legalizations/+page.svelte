<script lang="ts">
	...
	const rows = $derived(
		(data?.content ?? []).filter((row) => row.status === 'DESEMBOLSADA'),
	);
...
{#snippet rowActions(row: TravelRequestSummary)}
	<div class="flex items-center justify-end gap-0.5">
		<Button
			...
			variant="ghost"
			size="icon-sm"
			aria-label={`Legalizar solicitud ${row.requestNumber}`}
		>
...
<div class="space-y-4">
	<PageHeader
		...
		description="Revisa las solicitudes desembolsadas que están listas para legalizar."
	/>
	...
	<TravelRequestTable
		...
		emptyDescription="No hay solicitudes desembolsadas pendientes de legalización en este momento."
	/>
</div>
