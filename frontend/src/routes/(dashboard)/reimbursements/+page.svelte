<script lang="ts">
	...
	const rows = $derived(
		(data?.content ?? []).filter(
			(row) => row.status === 'PENDIENTE_REEMBOLSO',
		),
	);
...
{#snippet rowActions(row: TravelRequestSummary)}
	<div class="flex items-center justify-end gap-0.5">
		<Button
			href={`${ROUTES.finance.reimbursements}/${row.id}`}
			variant="ghost"
			size="icon-sm"
			aria-label={`Registrar reembolso ${row.requestNumber}`}
		>
...
<div class="space-y-4">
	<PageHeader
		...
		description="Revisa las legalizaciones con dinero a favor del empleado que están listas para registrar el reembolso."
	/>
	...
	<TravelRequestTable
		...
		onRetry={handleRetry}
		{rowActions}
		emptyTitle="No hay reembolsos pendientes"
		emptyDescription="No hay legalizaciones pendientes de reembolso en este momento."
	/>
</div>
