<script lang="ts">
	...
	const rows = $derived(
		(data?.content ?? []).filter(
			(row) => row.status === 'PENDIENTE_DEVOLUCION',
		),
	);
...
{#snippet rowActions(row: TravelRequestSummary)}
	<div class="flex items-center justify-end gap-0.5">
		<Button
			href={`${ROUTES.finance.refunds}/${row.id}`}
			variant="ghost"
			size="icon-sm"
			aria-label={`Registrar devolución ${row.requestNumber}`}
		>
...
<div class="space-y-4">
	<PageHeader
		...
		description="Revisa las legalizaciones con excedente de dinero que están listas para registrar la devolución."
	/>
	...
	<TravelRequestTable
		...
		emptyDescription="No hay legalizaciones pendientes de devolución en este momento."
	/>
</div>
